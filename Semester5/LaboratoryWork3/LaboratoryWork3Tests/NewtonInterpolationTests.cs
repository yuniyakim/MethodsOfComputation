...
namespace LaboratoryWork3Tests
{
    ...
    {
        private NewtonInterpolation newtonInterpolation = new NewtonInterpolation();
        ...
        {
            var table1 = newtonInterpolation.NodesTable(left, right, amountOfNodes);
            ...
            var table2 = newtonInterpolation.NodesTable(left, right, 10);
        ...
        {
            var table = newtonInterpolation.NodesTable(left, right, amountOfNodes);
            var sortedTable = newtonInterpolation.SortedNodesTable(table, point, powerOfPolynomial);
        ...
        {
            var table = newtonInterpolation.NodesTable(left, right, amountOfNodes);
            var orderedTable = newtonInterpolation.SortedNodesTable(table, point, powerOfPolynomial);
            var dividedDifferences = newtonInterpolation.DividedDifferences(orderedTable);

            Assert.That(newtonInterpolation.NewtonPolynomialValue(orderedTable, dividedDifferences), Is.EqualTo(Function(point)).Within(Math.Pow(10, -10)));
        }
    }
}